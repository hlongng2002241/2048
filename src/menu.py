...
from algorithm.mixed_expectmax import Mixed_Expectmax_Minimax
...
class Menu:
    def __init__(self, gameplay:GamePlay) -> None:
        ...
        form_algo.position          = (0, 101)
        Label("Algorithm:", font_size, 10, -4, form_algo)
        Button("Minimax", font_size, 0, 32, 355, 40, form_algo).callback        = lambda: self.gameplay.set_algorithm(Minimax(self.depth))
        Button("Minimax + Alpha Beta prune", font_size, 0, 77, 355, 40, form_algo).set_state(Button.TOGGLE).callback = lambda: self.gameplay.set_algorithm(MinimaxAlphaBeta(self.depth))
        Button("Expectmax", font_size, 0, 122, 355, 40, form_algo).callback     = lambda: self.gameplay.set_algorithm(Mixed_Expectmax_Minimax(self.depth))

        form_move                   = Form(self.menu)
        form_move.position          = (0, 290)
        Label("Moves per second:", font_size, 10, -5, form_move)
        Button("5", font_size, 0, 31, 67, 40, form_move).callback               = lambda: self.gameplay.set_moves_per_second(5)
        Button("10", font_size, 72, 31, 67, 40, form_move).set_state(Button.TOGGLE).callback = lambda: self.gameplay.set_moves_per_second(10)
        Button("20", font_size, 144, 31, 67, 40, form_move).callback            = lambda: self.gameplay.set_moves_per_second(20)
        Button("100", font_size, 216, 31, 67, 40, form_move).callback           = lambda: self.gameplay.set_moves_per_second(100)
        Button("200", font_size, 288, 31, 67, 40, form_move).callback           = lambda: self.gameplay.set_moves_per_second(200)

        form_depth                  = Form(self.menu)
        form_depth.position         = (0, 387)
        Label("Max depth:", font_size, 10, -5, form_depth)
        Button("1", font_size, 0, 32, 85, 40, form_depth).callback              = lambda: self._set_depth(1)
        Button("2", font_size, 90, 32, 85, 40, form_depth).callback             = lambda: self._set_depth(2)
        Button("3", font_size, 180, 32, 85, 40, form_depth).callback            = lambda: self._set_depth(3)
        Button("4", font_size, 270, 32, 85, 40, form_depth).set_state(Button.TOGGLE).callback = lambda: self._set_depth(4)
        Button("5", font_size, 0, 77, 85, 40, form_depth).callback              = lambda: self._set_depth(5)
        Button("6", font_size, 90, 77, 85, 40, form_depth).callback             = lambda: self._set_depth(6)
